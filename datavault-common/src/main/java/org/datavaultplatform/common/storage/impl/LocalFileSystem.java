...
public class LocalFileSystem extends Device implements UserStore, ArchiveStore {
  ...
  public LocalFileSystem(String name, Map<String, String> config)
    ...
    Assert.isTrue(
      file.isDirectory(),
      () -> String.format("rootPath is not a directory[%s]", rootPath)
    );
    Assert.isTrue(
      file.canRead(),
      () -> String.format("rootPath is not readable[%s]", rootPath)
    );
    Assert.isTrue(
      file.canWrite(),
      () -> String.format("rootPath is not writable[%s]", rootPath)
    );
  }
  ...
  public List<FileInfo> list(String path) {
    ...
    try (
      DirectoryStream<Path> stream = Files.newDirectoryStream(completePath)
    ) {
      for (Path entry : stream) {
        ...
        String entryKey =
          (basePath.toUri().relativize(entry.toUri())).getPath();
  ...
  public String store(String path, File working, Progress progress)
    throws Exception {
    Path absolutePath = getAbsolutePath(path);
    File retrieveFile = new File(
      absolutePath.resolve(working.getName()).toUri()
    );
