...
@Slf4j 
abstract class BasePerformDepositThenDeleteTwoArchivesIT extends BaseDepositTwoArchivesIT implements DepositThenDelete {
  ...
  private List<File> getExpectedDepositFiles(String depositMessage) {
    ...
    List<Path> baseDestinations = List.of(this.destDir1.toPath(), this.destDir2.toPath());
    List<File> files = new ArrayList<>();
    for (Path baseDestination : baseDestinations) {

      if (chunkingEnabled) {
        int numberOfChunks = getExpectedNumberChunksPerDeposit().orElseThrow();
        for (int chunkNum = 1; chunkNum <= numberOfChunks; chunkNum++) {
          File chunkTarFile = baseDestination.resolve(bagId + ".tar." + chunkNum).toFile();
          files.add(chunkTarFile);
        }
      } else {
        File singleTarFile = baseDestination.resolve(bagId + ".tar").toFile();
  ...
  @Override
  public List<ArchiveStoreInformation> getArchiveStoreInformation() {
    return List.of(
            new ArchiveStoreInformation(LocalFileSystem.class,"ARCHIVE-STORE-DST-ID-1"),
            new ArchiveStoreInformation(LocalFileSystem.class,"ARCHIVE-STORE-DST-ID-2"));
  }
