...
/* This code has been used to test the rebuild of a tar file from a set of encrypted chunks.
 * It does duplicate the work done in the 'org.datavaultplatform.worker.tasks.Retrieve' class but
 * this code is meant to be used for testing of the decryption process.
 *
 * This code performs decryption of encrypted chunks and
 * recomposition of decrypted chunks into a tar file.
 *
 */
@Slf4j
public class DecryptedTarBuilder {
  ...
  public File rebuildTarFile() {
    ...
    File tarDir = new File(tarBaseDir, "tardir_" + getTimestamp());
    ...
    chunks.forEach((chunkNum, chunkInfo) -> {
      ...
      byte[] ivBytesData = chunkData.getIvBytes();
      final byte[] ivBytes;
      if (ivBytesData != null && ivBytesData.length >= 0) {
        ivBytes = ivBytesData;
      } else {
        ...
        ivBytes = Base64.getDecoder().decode(iv.getBytes(StandardCharsets.UTF_8));
      }
      //for GCM - the iv byte should have length 96
      Assert.isTrue(ivBytes.length == 96,
          () -> String.format("The chunk[%d]iv byte array should be 96 length - it is [%d]",
              chunkNum, ivBytes.length));

      chunkInfo.setIvBytes(ivBytes);

      if (StringUtils.isNotBlank(chunkData.getIvDigest())) {
        String actualIvDigest = Encryption.getDigestForIv(ivBytes);
  ...
  private void setupEncryption(DecryptedTarBuilderParams params) {
    boolean isBlankKeyStorePath = StringUtils.isBlank(params.getKeystorePath());
    boolean isBlankKeyStorePassword = StringUtils.isBlank(params.getKeystorePassword());
    boolean isBlankKeyDataKeyName = StringUtils.isBlank(params.getDataKeyName());
    if (isBlankKeyStorePath && isBlankKeyStorePassword && isBlankKeyDataKeyName) {
      log.warn("not setting up encryption");
      return;
    }
