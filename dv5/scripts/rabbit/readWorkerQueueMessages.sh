...
. ./setup.sh
PAYLOAD_FILE="workerQueueMessages-$(date +%T).txt"
echo "Reading Worker Queue Message from Q[datavault] to file named [$PAYLOAD_FILE]"
BASE_Q_URL="http://localhost:15672/api/queues/%2F/datavault"
...
echo "read ${COUNT} messages from [datavault] queue"