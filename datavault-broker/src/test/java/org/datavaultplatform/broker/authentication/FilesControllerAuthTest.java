...
public class FilesControllerAuthTest extends BaseControllerAuthTest {
  ...
  @Captor
  ArgumentCaptor<String> argFilePath;
  ...
  void testGetFilesListing() throws Exception {
    ...
    when(controller.getFilesListing(argUserId.capture(),
        argStorageId.capture(), argFilePath.capture())).thenReturn(
    ...
    verify(controller).getFilesListing(argUserId.getValue(), 
        argStorageId.getValue(), argFilePath.getValue());
    assertEquals(USER_ID_1, argUserId.getValue());
  ...
  void testGetFileSize() throws Exception {
    ...
    when(controller.getFilesize(argUserId.capture(),
        argStorageId.capture(), argFilePath.capture())).thenReturn("file-size");

    checkWorksWhenAuthenticatedFailsOtherwise(
        get("/filesize/{storageid}/{filePath}", "storage-id-xxx", "a/b/c"),
        "file-size");

    verify(controller).getFilesize(argUserId.getValue(),
        argStorageId.getValue(), argFilePath.getValue());
    assertEquals(USER_ID_1, argUserId.getValue());
    assertEquals("storage-id-xxx", argStorageId.getValue());
    assertEquals("/a/b/c", argFilePath.getValue());
  }

  @Test
  void testGetStorageListing() {

    when(controller.getStorageListing(argUserId.capture())).thenReturn(
    ...
    verify(controller).getStorageListing(argUserId.getValue());
    assertEquals(USER_ID_1, argUserId.getValue());
  }
