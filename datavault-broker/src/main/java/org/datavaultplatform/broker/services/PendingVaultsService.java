...
public class PendingVaultsService {
  ...
  @Autowired
  public PendingVaultsService(
    PendingVaultDAO pendingVaultDAO,
    GroupsService groupsService,
    RetentionPoliciesService retentionPoliciesService,
    UsersService usersService,
    RolesAndPermissionsService permissionsService,
    PendingDataCreatorsService pendingDataCreatorsService,
  ...
  public void addOrUpdatePendingVault(PendingVault vault) {
    ...
    if (vault != null) {
      if (vault.getId() == null || vault.getId().isEmpty()) {
        ...
        logger.info(
          "Updating an existing pending vault (" + vault.getId() + ")"
        );
  ...
  public void delete(String id) {
    ...
    // for each role
    List<RoleAssignment> roles = permissionsService.getRoleAssignmentsForPendingVault(
      id
    );
  ...
  public int getTotalNumberOfPendingVaults(
    ...
    return pendingVaultDAO.getTotalNumberOfPendingVaults(
      userId,
      query,
      confirmed
    );
  ...
  public List<PendingVault> search(
    String userId,
    String query,
    String sort,
    String order,
    ...
    return this.pendingVaultDAO.search(
        userId,
        query,
        sort,
        order,
  ...
  public PendingVault getUserPendingVault(User user, String vaultID)
    throws Exception {
  ...
  public void addCreator(
    ...
    pendingVaultCreatorRoleAssignment.setRole(
      permissionsService.getVaultCreator()
    );
    ...
    logger.debug(
      "Role type: '" +
      pendingVaultCreatorRoleAssignment.getRole().getType() +
      "'"
    );
  ...
  public PendingVault processDataCreatorParams(
    CreateVault createVault,
    PendingVault vault
  ) {
  ...
  public PendingVault processVaultParams(
    PendingVault vault,
    CreateVault createVault,
    String userID
  ) throws Exception {
    ...
    if (billingType != null && !billingType.isEmpty()) {
      PendingVault.Billing_Type enumBT = PendingVault.Billing_Type.valueOf(
        billingType
      );
      if (enumBT != null) {
    ...
    String policyId = (createVault.getPolicyInfo() != null)
      ? createVault.getPolicyInfo().split("-")[0]
      : null;
    logger.debug("Retention policy id is: '" + policyId + "'");
    if (policyId != null) {
      RetentionPolicy retentionPolicy = retentionPoliciesService.getPolicy(
        policyId
      );
      ...
      if (retentionPolicy == null) {
        ...
        throw new Exception(
          "RetentionPolicy '" + policyId + "' does not exist"
        );
      }
    ...
    if (fundingQueryChoice != null && !fundingQueryChoice.isEmpty()) {
      vault.setFundingQueryChoice(
        Funding_Query_Choice.valueOf(fundingQueryChoice)
      );
    } else {
    ...
    if (feewaiverQueryChoice != null && !feewaiverQueryChoice.isEmpty()) {
      vault.setFeewaiverQueryChoice(
        Feewaiver_Query_Choice.valueOf(feewaiverQueryChoice)
      );
    } else {
  ...
  public void sendNewPendingVaultEmail(PendingVault vault) {
    ...
    this.emailService.sendTemplateMail(
        this.helpMail,
        "New Pending Vault Submitted",
        EmailTemplate.GROUP_ADMIN_PV_SUBMITTED,
        model
      );
  }
}
