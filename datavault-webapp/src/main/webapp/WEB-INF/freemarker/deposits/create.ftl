        ...
        <div class="panel-body">
            ...
            <form id="create-deposit" class="form" role="form" action="" method="post">
                ...
                <div class="flow-drop">
                    ...
                    <p><label id="deposit-size" class="text-muted"></label></p>
                </div>
                
                <script>
                    var filesizeSeq = 0;

                    const sortTreeNodesFn = function (node) {
                    ...
                    $("#tree").fancytree({
                        ...
                        click: function(event, data){
                            // Enables clicking on titles, not just checkbox
...
<script>
    $(function () {
        ...
        const updateProgress = function(percentComplete) {
        ...
        $('#add-from-storage-btn').on("click", function() {
            console.log("#add-from-storage-btn: CLICKED");
            const tree = $.ui.fancytree.getTree("#tree");
            console.log("#tree: ", tree);
            const selectedNodes = Array.from(new Set(tree.getSelectedNodes()));
            console.log("selectedNodes: ", selectedNodes);
            
            const uploadTree = $.ui.fancytree.getTree("#upload-tree");
            console.log("#upload-tree: ", uploadTree);
            var storageNode = uploadTree.getNodeByKey("storage", uploadTree.rootNode)
            console.log("storageNode: ", storageNode);
            var paths = [];
            const pathsSet = new Set();
            if (storageNode) {
                const pathNodes = storageNode.getChildren();
                var isFull = false;
                if (pathNodes != null) {
                    for (var i = 0; i < pathNodes.length; i++) {
                        const checkedNode = pathNodes[i];
                        pathsSet.add(checkedNode.getKeyPath().replace("/storage/", "/"));
                    }
                }
            }
            ...
            $.map(selectedNodes, function(node) {
                ...
                pathsSet.add(node.key);
            });
            console.log("pathsSet: ", pathsSet);
            // To get rid of duplicates, we obtain a set from paths and covert back to an array
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if(!isFull) {
                        // Clear file-path and storageNode
                        $('.file-path').empty(); 
                        if (storageNode) {
                           storageNode.removeChildren();
                        }
                        
                        // Loop over selectedNodes array, but first remove duplicates.
                        $.map(selectedNodes, function(node) {

                            // Get upload tree and its root node
                            const uploadTree = $.ui.fancytree.getTree("#upload-tree");
                            const rootNode = uploadTree.rootNode;
                            ...
                            $('.file-path').append($('<option>', { value : node.key })
                            .text(node.key)
                            .prop('selected', true));
                        ...
                        $("#deposit-size").text("Deposit size: " + result.sizeWithUnits);
                        //Show tree
                        $("#upload-tree").show();
                    } else {
                        alert("A deposit is limited to " + result.max + "!\nYour deposit is of size: " + result.sizeWithUnits);
                    }
                }
            })
            .fail(function( msg ) {
        ...
        const modalContentMinWidth = 350;
        const modalContentMinHeight = modalContentMinWidth*0.7;
