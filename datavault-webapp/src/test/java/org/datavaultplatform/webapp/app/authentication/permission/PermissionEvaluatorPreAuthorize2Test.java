 ...
 */
class PermissionEvaluatorPreAuthorize2Test {
    ...
    class UsingPermissionEvaluator {
      ...
      class PathTwoUsesHasPermissionThreeArgs {
        ...
        void test(ApplicationContext ctx) {
          String[] evaluators = ctx.getBeanNamesForType(PermissionEvaluator.class);
          Arrays.stream(evaluators).forEach(eval -> {
            System.out.printf("EVALUATOR [%s] evaluator", eval);
          });
        }
